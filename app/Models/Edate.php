...
class Edate extends Model
{
    ...
    public static function j2carbon($jdate = '1300-01-01')
    {
        $x = explode('-', $jdate);
        $ts = Edate::jmktime(6, 0, 0, $x[1], $x[2], $x[0]);
        return Carbon::createFromTimestamp($ts);
    }

    public static function edate($format, $timestamp = null, $tr_num = 'en')
    {
        if (is_null($timestamp)) $timestamp = time();
        return self::jdate($format, $timestamp, '', '', $tr_num);
    }

    public static function edateFromCarbon($format, $carbonDate = null, $tr_num = 'en')
    {
        if (is_null($carbonDate)) return '';
        if (Useful::str_has($carbonDate, 'T')) {
            $x = explode('T', $carbonDate);
            $carbonDate = $x[0] . ' ' . explode('.', $x[1])[0];
        }
        $explode = explode(' ', $carbonDate);
        ...
        if ($format == 'fluent') $format = 'j F Y' . '، ساعت ' . 'H:i';
        return self::jdate($format, $timestamp, '', '', $tr_num);
    }

    public static function date_diff_carbon($date, $reference = null, $mode = null)
    {
        $reference ? $a = Carbon::parse($reference)->timestamp : $a = time();
        $b = Carbon::parse($date)->timestamp;
        $diff = $a - $b;
        $res = '';
        if ($mode == 'day')
            return floor($diff / 86400);
        $years = floor($diff / (365 * 86400));
        if ($years > 0) $res .= $years . ' سال و ';
        $months = floor(($diff % (365 * 86400)) / (30 * 86400));
        if ($months > 0) $res .= $months . ' ماه و ';
        $days = floor((($diff % (365 * 86400)) % (30 * 86400)) / 86400);
        if ($days > 0) $res .= $days . ' روز';
        if (mb_substr($res, -1, null, 'UTF-8') == ' ') $res = mb_substr($res, 0, -3, 'UTF-8');
        return $res;
    }

    public static function this_month_span()
    {
        $date = self::edate('Y-m-t');
        $date = explode('-', $date);
        $Y = $date[0];
        $m = $date[1];
        $d_end = $date[2];
        return [$Y . '-' . $m . '-01', $Y . '-' . $m . '-' . $d_end];
    }

    public static function last_month_span()
    {
        $date = self::edate('Y-m');
        $date = explode('-', $date);
        $Y = $date[0];
        ...
        if ($m == 0) {
            $Y = $Y - 1;
            $m = 12;
            self::jcheckdate($m, 30, $Y) ? $d_end = 30 : $d_end = 29;
        } else if (Useful::is_between($m, 1, 6)) {
            $d_end = 31;
        } else {
            $d_end = 30;
        }
        return [
            $Y . '-' . Useful::zerofill($m, 2) . '-01',
            $Y . '-' . Useful::zerofill($m, 2) . '-' . $d_end,
            self::jdate_words(['mm' => $m])['mm']
        ];
    }

    public static function last_year_span()
    {
        $date = self::edate('Y');
        $Y = $date - 1;
        self::jcheckdate(12, 30, $Y) ? $d_end = 30 : $d_end = 29;
        return [$Y . '-01-01', $Y . '-12-' . $d_end];
    }
}
