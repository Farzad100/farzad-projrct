...
export default {
  ...
  methods: {
    async loadLine(payload, filter) {
      // HTTP Request
      const { data } = await wrapper(api.Admin.charts.installment.line(payload, filter));
    ...
    async loadPie() {
      // HTTP Request
      const { data } = await wrapper(api.Admin.charts.installment.pie());
    ...
    async loadOverview() {
      const { data } = await wrapper(api.Admin.charts.installment.overview());
