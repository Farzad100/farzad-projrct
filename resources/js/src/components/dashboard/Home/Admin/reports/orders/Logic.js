...
export default {
  ...
  methods: {
    async loadLine(payload, filter) {
      // HTTP Request
      const { data } = await wrapper(api.Admin.charts.orders.line(payload, filter));
    ...
    async loadPie() {
      // HTTP Request
      const { data } = await wrapper(api.Admin.charts.orders.pie());
    ...
    async loadOverview() {
      const { data } = await wrapper(api.Admin.charts.orders.overview());
