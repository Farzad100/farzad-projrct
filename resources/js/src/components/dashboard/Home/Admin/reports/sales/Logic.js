...
export default {
          ...
          tooltip: {
            y: {
              formatter: function(value) {
                return `${value.toLocaleString()} <span class="opa-5 font-weight-light">تومان</span>`;
              }
            },
            x: {
              show: true,
  ...
  methods: {
    async loadLine(payload, filter) {
      // HTTP Request
      const { data } = await wrapper(api.Admin.charts.sales.line(payload, filter));
      ...
      if (data) {
        ...
        for (let i in series[0].data) {
          total.data.push(
            [
              series[0].data[i][0],
              parseInt([series[0].data[i][1]]) + parseInt([series[1].data[i][1]]) + parseInt([series[2].data[i][1]])
            ]
          );
        }
    ...
    async loadPie() {
      // HTTP Request
      const { data } = await wrapper(api.Admin.charts.sales.pie());
    ...
    async loadOverview() {
      const { data } = await wrapper(api.Admin.charts.sales.overview());
